...
The THINK C dumper ([DumpGenericM68kMacCode.py](../DumpGeneric68kMacCode.py)) does not generally produce a usable dump for CodeWarrior programs. This is because many such programs use the default CodeWarrior startup code that does nasty things, including constructing the A5 world from a compressed resource (first part of `DATA 0`), decompressing relocation information from last part of `DATA 0`, and then relocating code segments on-demand with a custom `LoadSeg`.
