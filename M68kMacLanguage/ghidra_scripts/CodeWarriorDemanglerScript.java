...
public class CodeWarriorDemanglerScript extends GhidraScript {
    ...
    private static class CodeWarriorDemangler {
        public String str;
        ...
        public boolean isEmpty() {
            return this.str == null || this.str.length() < 1;
        }

        private String cw(int n) {
            String g = this.str.substring(0, n);
            this.str = this.str.substring(n);
            return g;
        }

        private char hd() {
            return isEmpty() ? 0 : this.str.charAt(0);
        }

        private boolean isConstFunc() {
            return (isEmpty() || this.str.length() < 2) ? false : this.str.startsWith("CF") || this.str.startsWith("cF");
        }
        ...
        public static DemangledObject demangleSymbol(String symbolName) {
            ...
            // If the symbol merely starts with __, without another one, exit.
            if (firstDunder < 0)
                return null;
        ...
        private static String demangleSpecialOperator(String symbolName) {
            if (symbolName.startsWith("__")) {
                switch (symbolName.substring(2)) {
                    case "nw": return "operator new";
                    case "nwa": return "operator new[]";
                    case "dl": return "operator delete";
                    case "dla": return "operator delete[]";
                    case "pl": return "operator +";
                    case "mi": return "operator -";
                    case "ml": return "operator *";
                    case "dv": return "operator /";
                    case "md": return "operator %";
                    case "er": return "operator ^";
                    case "ad": return "operator &";
                    case "or": return "operator |";
                    case "co": return "operator ~";
                    case "nt": return "operator !";
                    case "as": return "operator =";
                    case "lt": return "operator <";
                    case "gt": return "operator >";
                    case "apl": return "operator +=";
                    case "ami": return "operator -=";
                    case "amu": return "operator *=";
                    case "adv": return "operator /=";
                    case "amd": return "operator %=";
                    case "aer": return "operator ^=";
                    case "aad": return "operator &=";
                    case "aor": return "operator |=";
                    case "ls": return "operator <<";
                    case "rs": return "operator >>";
                    case "ars": return "operator >>=";
                    case "als": return "operator <<=";
                    case "eq": return "operator ==";
                    case "ne": return "operator !=";
                    case "le": return "operator <=";
                    case "ge": return "operator >=";
                    case "aa": return "operator &&";
                    case "oo": return "operator ||";
                    case "pp": return "operator ++";
                    case "mm": return "operator --";
                    case "cl": return "operator ()";
                    case "vc": return "operator []";
                    case "rf": return "operator ->";
                    case "cm": return "operator ,";
                    case "rm": return "operator ->*";
                }
            }
            return null;
        }
    ...
    protected void run() throws Exception {
        ...
        for (Symbol symbol : symbolTable.getAllSymbols(true)) {
            ...
            // Skip if not a valid mangled name candidate
            if (mangledName == null || mangledName.length() < 3 || !mangledName.contains("__")) {
                continue;
            }

            try {
                // Attempt to demangle
                DemangledObject demangled = CodeWarriorDemangler.demangleSymbol(mangledName);

                if (demangled != null) {
                    // Apply the demangled name and signature
                    if (demangled.applyTo(currentProgram, symbol.getAddress(), new DemanglerOptions(), monitor)) {
                        demangledCount++;
                        printf("Demangled: %s -> %s\n", mangledName, demangled.getName());
                    }
                }
            } catch (Exception e) {
                errorCount++;
                printf("Error demangling %s: %s\n", mangledName, e.getMessage());
            }
        }
